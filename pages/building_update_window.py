...
class BuildingUpdateWindow(QtWidgets.QWidget):
    def __init__(self, parent, data_manager:DataManager, building=None):
        super().__init__()
    ...
    def _on_save_click(self):
        ...
        if self.building_index is not None:
            self.data[self.building_index]['name'] = name
        else:
            self.data.append(
                { "name": name, "items": [] }
            )
    ...
    def _create_widgets(self):
        ...
        if self.building_index is not None:
            self.name_input.setText(self.data[self.building_index]['name'])
